...
#include "ui_zeei_wm_dataquery.h"
...
ZEEI_WM_DataQuery::ZEEI_WM_DataQuery(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::ZEEI_WM_DataQuery)
{
    ui->setupUi(this);
    this->initPage();
    this->initForm();
...
ZEEI_WM_DataQuery::~ZEEI_WM_DataQuery()
{
    delete ui;
}

void ZEEI_WM_DataQuery::initPage()
{
    ui->widgetLeft->setProperty("form", "dataQueryLeft");
...
void ZEEI_WM_DataQuery::initForm()
{    
    columnNames.clear();
    columnWidths.clear();
    columnValue = "Value";
...
void ZEEI_WM_DataQuery::initFidCmb()
{
    uci myuci;
...
void ZEEI_WM_DataQuery::on_btnSelect_clicked()
{
    qDebug()<<"query";
...
void ZEEI_WM_DataQuery::showChanged(int index)
{
    ui->stackedWidget->setCurrentIndex(index);
}

//区域选择Check状态改变
void ZEEI_WM_DataQuery::stateChanged(int state)
{
    bSelected = true;
...
//区域选择内容改变
void ZEEI_WM_DataQuery::textChanged(const QString &text)
{
    if (!bSelected)
        pLineEdit->setText(strSelectedText);
}

void ZEEI_WM_DataQuery::on_cmbMode_currentIndexChanged(int index)
{
    switch (index) {
    case 0:
